...
let cleanupActiveMenu: (() => void) | null = null
...
export function showTerminalContextMenu(event: MouseEvent, items: TerminalContextMenuItem[]) {
  ...
  const closeMenu = (nextEvent: Event) => {
    if (!menu.contains(nextEvent.target as Node)) {
      cleanup()
    }
  }
