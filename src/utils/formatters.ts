export function formatBytes(bytes?: number, options: { emptyValue?: string } = {}) {
  const emptyValue = options.emptyValue ?? '0 B'
  if (!bytes || bytes === 0) return emptyValue
...
export function formatUnixTimestamp(seconds?: number, options: { emptyValue?: string } = {}) {
